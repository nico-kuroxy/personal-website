# Kuroxy's personal website

The website is currently deployed at: https://nico-kuroro.xyz

Each new commit to the master branch triggers a redeployment on cloudflared's side.

## Todo

 - [x] Develop modal  
 - [x] Deploy the server on the server at home  
 - [x] Clean the plot data component  
 - [x] Fix the plot data when stopping / panning : data highlighted with the vertical bar does not match the visual plot  
 - [x] Add full zoom
 - [x] Develop guide modal for the homepage.
 - [x] Develop dark/light mode.
 - [x] Investigate why the switch to the homepage sometimes takes forever...
 - [x] Develop language switch.
 - [x] Add JP language support.
 - [x] Add apriltag support.
 - [ ] Develop interaction with the urdf (hightlight relevant components highlight focused component, hightlight component on hover and allow to click on it)  
 - [ ] Separate vertical and horizontal zoom.
 - [ ] Replace picture of medalion and fix lighting.

