...
export default function RobotCard(props) {
    ...
    return (
        ...
        <div style={{...style, top: '0%', left: '15%', transform: `translate(${style.left}, ${style.top})`}}
            className={`flex flex-col text-left w-[75%] h-[50vh] absolute cursor-pointer rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.6)] transition-[transform] duration-500 ease-in-out hover:scale-105 p-4 `
                + (orderIndex === -1? " bg-orange-500 " : orderIndex === 2? " bg-blue-500 text-white " : " text-black bg-white hover:bg-yellow-300 " )}
             onClick={onClick}>
            { /* The card title */}
